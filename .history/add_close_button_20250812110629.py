...
def update_project_file(filename):
    ...
    try:
        ...
        if re.search(css_pattern, content, re.DOTALL):
            content = re.sub(css_pattern, css_replacement, content, count=1, flags=re.DOTALL)
        ...
        if re.search(html_pattern, content):
            ...
            print(f"Warning: Could not find <div class=\"project-content\"> pattern in {filename}")
